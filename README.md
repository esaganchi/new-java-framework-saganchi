...
### Запуск с Docker
```bash
# Запуск всей инфраструктуры (Selenium Grid + тесты)
...
- **Docker интеграция**: полная контейнеризация тестовой среды
- **Jenkins CI/CD**: готовый пайплайн для автоматизации

## 🐳 Docker конфигурация

Проект включает полную Docker инфраструктуру:

- **Dockerfile** - контейнеризация тестового приложения
- **docker-compose.yml** - оркестрация всех сервисов:
  - Selenium Hub (порт 4444)
  - Chrome nodes (масштабируемые)
  - Firefox node
  - Test runner
  - Allure server (порт 5050)

### Полезные Docker команды
```bash
# Просмотр логов Selenium Grid
docker-compose logs selenium-hub chrome firefox

# Масштабирование Chrome nodes
...
Проект содержит готовый `Jenkinsfile` с функциями:

- **Параметризованные сборки**: выбор браузера, типа тестов
- **Docker интеграция**: автоматический подъем Selenium Grid
- **Allure отчеты**: автоматическая генерация и публикация
- **Уведомления**: детальные результаты тестирования
- **Артефакты**: сохранение отчетов и логов

### Настройка Jenkins
1. Создайте новый Pipeline проект
2. Укажите репозиторий с Jenkinsfile
3. Настройте параметры сборки
4. Установите плагины: Allure, HTML Publisher
